...
#### Map Asset Generation (World Map)
- Отказались от рендера «живой» SVG карты в момент экспорта PDF (браузеры плохо скейлят SVG на несколько листов во время `print()`).
- Внедрён сервисный роут `/?map`, который рендерит только карту.
- Написан скрипт `scripts/export-map.mjs` (вызов через `npm run map`). Он использует Puppeteer (в фоне) для генерации PNG в 4-кратном разрешении (2800×1604px) из локально запущенного React приложения.
- Карта `assets/images/map-4x.png` теперь просто статично вставляется в `TopCountriesSlide`, гарантируя идеальное качество как в браузере, так и в PDF.

#### Browser PDF Export & GitHub Pages
- Удалён нестабильный бекенд-экспорт в PDF.
- Кнопка **Export PDF** теперь ведёт на `/?pdf=all` (где слайды выстроены вертикальной стопкой). Работает через нативный вызов `window.print()`.
- Добавлен CSS `@page { size: 720px 450px; margin: 0; }` для идеального вписывания в листы.
- Оптимизирован `vite.config.ts` (`base: '/Watch360-PDF-Reports/'`) для хостинга из подпапок.
- Приложение полностью задеплоено на **GitHub Pages** как статический сайт (бесплатно, без бекенда).
- Исправлен баг "404", добавив вызов `window.location.pathname + '?pdf=all'`, чтобы URL склеивался относительно папки деплоя. 
